...
function test_main_file_lsp()
  local main_config = make_lsp_config(
    paths.main_file,
    "main program",
    { pattern = "get_hello_message", column_offset = 0 }
  )
  ...
  luaunit.assertEquals(result.client.name, "clangd", "Should use clangd server")
...
function test_test_file_lsp()
  local test_config = make_lsp_config(
    paths.test_file,
    "test file",
    { pattern = "test_get_hello_message", column_offset = 0 }
  )
