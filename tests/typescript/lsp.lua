...
if vim.g.__typescript_lsp_e2e_running then return end
vim.g.__typescript_lsp_e2e_running = true
...
if vim.fn.filereadable(tsconfig_file) == 0 then die("TypeScript config not found: " .. tsconfig_file) end

-- Enter project root for consistent ts_ls root detection
local prev_cwd = uv.cwd()
pcall(vim.fn.chdir, project_root)

-- Function to test LSP functionality for a TypeScript file
local function test_typescript_lsp(file_path, file_type)
  print("Testing TypeScript LSP for " .. file_type .. " ...")
  
  -- Open file to trigger ts_ls
  vim.cmd("edit! " .. vim.fn.fnameescape(file_path))
  if vim.bo.filetype ~= "typescript" then
    pcall(vim.fn.chdir, prev_cwd)
    die("Expected filetype=typescript for " .. file_type .. ", got " .. tostring(vim.bo.filetype))
  end
  ...
  print("✅ TypeScript LSP working for " .. file_type)
...
test_typescript_lsp(main_file, "main module")

-- Test TypeScript test file
test_typescript_lsp(test_file, "test module")
...
print("")
print("🎉 TypeScript LSP tests completed successfully!")
print("   Files tested: helloworld.ts, helloworld.test.ts")
